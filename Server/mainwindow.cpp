...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    {
        QMessageBox::critical(this,"Server",QString("서버가 시작되지 않음: %1.").arg(m_server->errorString()));
        exit(EXIT_FAILURE);
    }
}
...
void MainWindow::readSocket()
{
    ...
    if(fileType=="attachment"){ // 메시지 유형이 "첨부파일"인 경우
        ...
        if (QMessageBox::Yes == QMessageBox::question(this, "Server", QString("파일을 받겠습니까?").arg(socket->socketDescriptor()).arg(size).arg(fileName))) // 파일을 받는 경우
        {
            ...
            if(file.open(QIODevice::WriteOnly)){
                file.write(buffer);
                QString message = QString("%2 보낸 파일이 저장되었습니다.").arg(socket->socketDescriptor()).arg(QString(filePath));
                emit newMessage(message);
            }else  // 오류 발생
                QMessageBox::critical(this,"Server", "오류발생");
        }else{// 파일을 받지 않는 경우
...
void MainWindow::discardSocket() // 소켓 삭제
{
    ...
    if (it != connection_set.end()){
        displayMessage(QString("접속 종료.").arg(socket->socketDescriptor()));
...
void MainWindow::displayError(QAbstractSocket::SocketError socketError)
{
    switch (socketError) {
        ...
        case QAbstractSocket::HostNotFoundError:
            QMessageBox::information(this, "Server", "호스트가 없습니다.");
        break;
        case QAbstractSocket::ConnectionRefusedError:
            QMessageBox::information(this, "Server", "호스트 이름 및 포트 설정이 올바른지 확인");
        break;
        default:
            QTcpSocket* socket = qobject_cast<QTcpSocket*>(sender());
            QMessageBox::information(this, "Server", QString("오류 발생: %1.").arg(socket->errorString()));
        break;
    }
}
...
void MainWindow::sendMessage(QTcpSocket* socket) // 메시지 전송
{
    if(socket)
    {
        if(socket->isOpen())
        {
            ...
            QMessageBox::critical(this,"Server","소켓이 열려있지 않음.");
    }
    else
        QMessageBox::critical(this,"Server","연결되지 않음.");
}
    ...
    if(socket)
    {
        if(socket->isOpen())
        {
            ...
            if(m_file.open(QIODevice::ReadOnly)){
                ...
                QMessageBox::critical(this,"Client","첨부파일을 열 수 없음.");
        }
        else
            QMessageBox::critical(this,"Server","소켓이 열려있지 않음.");
    }
